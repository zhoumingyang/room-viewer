export const spotLightRTXFragment: string = `#version 300 es
...
float SceneIntersect( Ray r, inout Intersection intersec ) {
	...
	for (int i = 0; i < N_DISKS; i++) {
		...
		if (d < t) {
			t = d;
			intersec.normal = dot(disks[i].normal,r.direction) <= 0.0 ? normalize(disks[i].normal) : normalize(disks[i].normal * -1.0);
			...
			toLightBulb = normalize(spheres[sp].position - hitPos);
	...
	for (int i = 0; i < N_OPENCYLINDERS; i++) {
		d = OpenCylinderIntersect( openCylinders[i].pos0, openCylinders[i].pos1, openCylinders[i].radius, r, n );
		if (d < t) {
			t = d;
			intersec.normal = normalize(n);
	...
	while (true) {
		if (currentStackData.rayT < t)  {
			//  < 0.0 signifies a leaf node
			if (currentBoxNode.branch_A_Index < 0.0) {
...
vec3 CalculateRadiance( Ray r, inout uvec2 seed ) {
	...
	Sphere light2 = spheres[3];
	Sphere lightChoice = light;
	float randChoose;
	vec3 accumCol = vec3(0.0);
    vec3 mask = vec3(1.0);
	vec3 checkCol0 = vec3(2.0);
...
void SetupScene(void) {
	vec3 z  = vec3(0);
	vec3 L1 = vec3(0.5, 0.7, 1.0) * 0.35;// Blueish sky light
	vec3 L2 = vec3(1.0, 1.0, 1.0) * 900.0;// Bright white light bulb
	
	spheres[0] = Sphere( 10000.0, vec3(0, 0, 0), L1, z, LIGHT);//large spherical sky light
	spheres[1] = Sphere( 3.0, vec3(-35, 100, -70), L2, z, SPOT_LIGHT);//small spherical light
	spheres[2] = Sphere( 4000.0, vec3(0, -4000, 0), z, vec3(0.1, 0.8, 0.4), CHECK);//Checkered Floor
	spheres[3] = Sphere( 3.0, vec3(35, 100, -10), L2, z, SPOT_LIGHT);//small spherical light
	...
	vec3 spotLightPos2 = spheres[3].position;
	vec3 spotLightDir2 = normalize(vec3(5, 20, -35) - spotLightPos2);
	openCylinders[1] = OpenCylinder( spotLightPos2 - (spotLightDir2 * spheres[3].radius) * 2.0, spotLightPos2 + (spotLightDir2 * spheres[3].radius) * 5.0, 
					   spheres[3].radius * 1.5, z, vec3(1), SPEC); //metal open Cylinder
	...
	disks[1] = Disk( spheres[3].radius * 1.5, spotLightPos2 - (spotLightDir2 * spheres[3].radius * 2.0), spotLightDir2, z, vec3(0.9, 0.9, 0.9), SPEC);//metal disk
	boxes[0] = Box( vec3(-60.0, 11.0, -110.0), vec3(60.0, 18.0, -20.0), z, vec3(0.65, 0.65, 0.65), REFR);//Glass Box
	boxes[1] = Box( vec3(-40.0, 13.0, -104.0), vec3(40.0, 16.0, -26.0), z, vec3(0, 0, 0), DIFF);//Inner Box
}
